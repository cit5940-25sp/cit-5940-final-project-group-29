...
public class GameView {
    ...
    private final int headerLines = 5;
    ...
    /**
     * Initializes the GameView, setting up the terminal and screen for display.
     *
     * @throws IOException If an error occurs while initializing the terminal or screen.
     */
    ...
    /**
     * Calculates the layout constants based on the screen size.
     */
    ...
    /**
     * Initializes the game by loading movie data and setting up players.
     *
     * @param moviesCsvPath Path to the CSV file containing movie data.
     * @param creditsCsvPath Path to the CSV file containing movie credits data.
    ...
    /**
     * Runs the main game loop, updating the screen and handling user input.
     *
    ...
    /**
     * Starts the player turn timer, counting down the remaining time for the current player's move.
     */
    ...
    /**
     * Stops the player turn timer when the turn is finished or interrupted.
     */
    ...
    /**
     * Handles the logic when the player runs out of time during their turn.
     */
    ...
    /**
     * Handles key strokes based on the current game phase, including link strategy choice, movie title input,
     * and displaying move results. Special keys like <code>Escape</code> to exit and <code>Enter</code> to proceed
     * are also processed.
     *
     * @param keyStroke The player's key stroke input.
     * @throws IOException If an error occurs during screen update or input processing.
     */
    private void handleKeyStroke(KeyStroke keyStroke) throws IOException {
        if (gameController == null && currentPhase != GamePhase.GAME_OVER) {
            return;
        }
    ...
    /**
     * Handles the input when the player chooses a link strategy.
     * This method processes the key stroke, verifies if it's valid,
     * and applies the selected strategy for the current round.
     *
     * @param keyStroke The key stroke input from the player.
     */
    ...
    /**
     * Handles input for movie title, including special keys like <code>Backspace</code>, <code>ArrowUp</code>, <code>ArrowDown</code>,
     * and <code>Enter</code>.
     *
     * @param keyStroke The key stroke input from the player.
    ...
    /**
     * Updates the movie suggestions based on the current input.
     */
    ...
    /**
     * Processes the guessed movie title, validating the input and handling the move accordingly.
     *
     * @param guessedMovieTitle The movie title entered by the player.
     * @throws IOException If an error occurs during the move processing.
     */
    ...
    /**
     * Updates the screen based on the current game state, including game phases and feedback messages.
     *
     * @throws IOException If an error occurs while updating the screen.
     */
    private synchronized void updateScreen() throws IOException {
        ...
        // Draw Feedback (if not already handled by SHOWING_MOVE_RESULT directly)
        ...
        // Draw Game History (at the bottom or after other content)
    ...
    /**
     * Draws the header and timer on the screen, including win condition and player info.
     *
     * @param size The terminal size to ensure proper layout.
     * @throws IOException If an error occurs while drawing the header or timer.
     */
    ...
    /**
     * Draws the link strategy choices for the player to select.
     *
     * @param startRow The starting row to display the link strategies.
     * @param size The terminal size.
     * @return The next available row after displaying the strategies.
     * @throws IOException If an error occurs while drawing the strategies.
     */
    ...
    /**
     * Draws the input layout for the movie title.
     *
     * @param startRow The starting row to display the movie input.
     * @param size The terminal size.
     * @return The next available row after the movie input.
     * @throws IOException If an error occurs while drawing the movie input.
     */
    private int drawMovieInputLayout(int startRow, TerminalSize size) throws IOException {
        int currentRow = startRow;
        if (currentRow >= size.getRows() -1) {
            return currentRow;
        }
    ...
    /**
     * Draws movie title suggestions based on the player's input.
     *
     * @param startRow The starting row to display the suggestions.
     * @param size The terminal size.
     * @return The next available row after the suggestions.
     * @throws IOException If an error occurs while drawing the suggestions.
     */
    ...
    /**
     * Draws the feedback message layout based on the game state.
     *
     * @param size The terminal size.
     * @param startRow The starting row to display the feedback message.
     * @return The next available row after the feedback message.
     * @throws IOException If an error occurs while drawing the feedback message.
     */
    private int drawFeedbackMessageLayout(TerminalSize size, int startRow) throws IOException {
        if (!feedbackMessage.isEmpty()) {
            ...
            if (actualFeedbackRow < 0 || actualFeedbackRow >= size.getRows()) {
                return startRow;  // Bounds check
            }
    ...
    /**
     * Draws the game history layout, displaying recent moves in the game.
     *
     * @param size The terminal size.
     * @param startRow The starting row to display the game history.
     * @throws IOException If an error occurs while drawing the history.
     */
    private void drawGameHistoryLayout(TerminalSize size, int startRow) throws IOException {
        ...
        if (actualStartRow >= size.getRows()) {
            return;
        }
        ...
        for (int i = startIndex; i < historySize; i++) {
            ...
            if (actualStartRow + linesDrawn >= size.getRows() -1 ) {
                break;
            }
            ...
            if (linesDrawn >= MAX_HISTORY_DISPLAYED * 2) {
                break;  // Limit lines drawn to avoid overflow
            }
        }
    }

    /**
     * Prints a string at a specified position with a color.
     *
     * @param x The x-coordinate.
     * @param y The y-coordinate.
     * @param text The text to be printed.
     * @param color The text color.
     * @throws IOException If an error occurs while printing the string.
     */
    ...
    /**
     * Prints a string at a specified position with foreground and background colors.
     *
     * @param x The x-coordinate.
     * @param y The y-coordinate.
     * @param text The text to be printed.
    ...
    private void printString(int x, int y, String text, TextColor fgColor, TextColor bgColor) throws IOException {
        ...
        if (terminalSize == null || y < 0 || y >= terminalSize.getRows()) {
            return;
        }
    ...
    /**
     * Prints a string at a specific position, centered horizontally.
     *
     * @param y The y-coordinate.
     * @param text The text to be printed.
     * @param color The text color.
    ...
    /**
     * Shuts down the game view and releases resources.
     *
     * @throws IOException If an error occurs while shutting down the screen or terminal.
     */
